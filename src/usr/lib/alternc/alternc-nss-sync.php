 ...
 * launch me as a cron as root every 5 minutes :) 
 */
...
// Set the user to have THIS shell : (don't change it again if a human changed it later.)
$newshell="/bin/false"; // could be /bin/false or /bin/nologin
...
if (!count($members)) {
    syslog(LOG_INFO,"Can't list AlternC accounts, exiting for now");
    @unlink($lock);
    exit();    
}
